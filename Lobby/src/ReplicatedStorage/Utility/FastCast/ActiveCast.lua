...
local function copyTable(tbl : table)
	local newTbl = {}
...
local function copyRaycastParams(rayParams : RaycastParams)
	local newRayParams = RaycastParams.new()
	newRayParams.CollisionGroup = rayParams.CollisionGroup
	newRayParams.FilterDescendantsInstances = rayParams.FilterDescendantsInstances
	newRayParams.FilterType = rayParams.FilterType
	newRayParams.IgnoreWater = rayParams.IgnoreWater
...
local function SimulateCast(cast: ActiveCast, delta: number, expectingShortCall: boolean)
	...
	cast.StateInfo.DistanceCovered += math.clamp(rayDisplacement, 5, 99e99) -- This clamp so it wouldn't infinitely bounce
	...
	if part and part ~= cast.RayInfo.CosmeticBulletObject then
		...
		if cast.RayInfo.CanPierceCallback == nil or (cast.RayInfo.CanPierceCallback ~= nil and cast.RayInfo.CanPierceCallback(cast, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject) == false) then
			...
			if false then -- replace will option of reflecting or piercing
...
-- Ctor
function ActiveCastStatic.new(caster: Caster, origin: Vector3, direction: Vector3, velocity: Vector3 | number, castDataPacket: FastCastBehavior, args): ActiveCast
	...
	if castDataPacket.CosmeticBulletProvider == nil then
		...
		if cast.RayInfo.CosmeticBulletObject ~= nil then
			cast.RayInfo.CosmeticBulletObject = cast.RayInfo.CosmeticBulletObject
			if cast.RayInfo.CosmeticBulletObject.ClassName == "Model" then
